AOS.init({ duration: 700, once: true, offset: 60 });
    ...
    window.addEventListener('scroll', () => {
      ...
      // Active nav
      const sections = ['beranda', 'lomba', 'galeri', 'sponsor', 'jadwal', 'tahapan', 'kontak'];
      let cur = '';
      sections.forEach(id => {
    ...
    function updateCountdown() {
      ...
      if (diff <= 0) { document.getElementById('countdown').innerHTML = '<div class="cd-box"><span class="cd-num">🎉</span><span class="cd-text">Hari H!</span></div>'; return; }
    ...
    let particles = [];
    function resizeCanvas() { canvas.width = window.innerWidth; canvas.height = canvas.closest('section').offsetHeight || window.innerHeight; }
    resizeCanvas();
      ...
      particles.forEach((p, i) => {
        particles.slice(i + 1).forEach(q => {
          ...
          if (dist < 100) {
            ...
            ctx.strokeStyle = '#4D99D3'; ctx.lineWidth = 0.5;
    ...
    const lombaData = [
      ...
      },
      {
        ...
        flyer: 'assets/flyer/lomba family hmkg.png',
        link: '',
    ...
    const galleryData = [
      ...
      { cat: 'dokumentasi', label: 'Dokumentasi Lapangan', color: '#FAC016', h: 300 },
    ...
    function renderGallery(filter) {
      gg.innerHTML = '';
      galleryData.filter(g => filter === 'semua' || g.cat === filter).forEach((g, i) => {
    ...
    // TIMELINE
    const jadwalData = [
      { day: '21', month: 'Jul', time: '07.45 WIB', name: 'Pembukaan Acara HMKG Upacara HMKG', loc: 'Kantor Pusat BMKG', status: 'highlight' },
      { day: '21', month: 'Jul', time: '08.00 WIB', name: 'Bazzar HMKG', loc: 'Kantor Pusat BMKG', status: 'upcoming' },
      { day: '21', month: 'Jul', time: '08.00 WIB', name: 'Pelaksanaan Perlombaan HMKG Hari ke-1', loc: 'Kantor BMKG Pusat', status: 'upcoming' },
      { day: '22', month: 'Jul', time: '08.00 WIB', name: 'Jalan Sehat & Senam Bersama', loc: 'Kantor Pusat BMKG', status: 'upcoming' },
      { day: '14', month: 'Agust', time: '08.00 WIB', name: 'Penutup Perlombaan HMKG Hari ke-19', loc: 'BMKG Pusat & Online', status: 'upcoming' },
      { day: '19', month: 'Agust', time: '08.00 WIB', name: 'Puncak Acara dan Penutupan HMKG 2026', loc: 'Kantor BMKG Pusat', status: 'highlight' },
    ];
    ...
    function animateCounter(el) {
      ...
      const t = setInterval(() => {
        ...
        el.textContent = (cur >= 1000 ? (cur / 1000).toFixed(1) + 'K' : Math.floor(cur)) + suf;
